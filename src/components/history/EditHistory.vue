<template>
  <div class="main">
    <h5>Update Service History</h5>
    <div class="align-content-center">
      <validation-observer v-slot="{ handleSubmit }">
        <form @submit.stop.prevent="handleSubmit(updateHistory)">
...
export default {
  ...
  methods: {
    ...
    getHistory(){
      HistoryService.retrieveHistoryById(this.$route.params.id).then(res =>{
        this.history = res.data;
      })
    },
    updateHistory() {
      HistoryService.updateHistory(this.$route.params.id, this.history).then(() => {
