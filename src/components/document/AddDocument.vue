...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      DocumentService.uploadDocumentByIppis(this.$store.state.auth.user.ippis, this.currentFile,
          event => {
            this.progress = Math.round((100 * event.loaded) / event.total);
          })
          .then(response => {
            this.message = response.data.message;
            return DocumentService.retrieveDocumentsByIppis(this.$store.state.auth.user.ippis);
          })
          .then(files => {
  ...
  mounted() {
    DocumentService.retrieveDocumentsByIppis(this.$store.state.auth.user.ippis).then(response => {
