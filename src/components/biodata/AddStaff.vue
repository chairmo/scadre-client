      ...
      <validation-observer v-slot="{ handleSubmit }">
        <form @submit.stop.prevent="handleSubmit(addStaff)">
          ...
          <div class="row form-group">
            ...
            <div class="col-xl-4">
              <validation-provider name="Phone Number" :rules="{required: true, digits: 11}"
                                   v-slot="validationContext">
...
export default {
  ...
  created() {
    // this.fetchData();
    const { email, ippis, firstName, lastName } = this.$store.state.auth.user
    this.staff.email = email
    this.staff.ippis = ippis
